...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .photo-col canvas, .photo-col img {
...
<body>
  <div class="pad">
    ...
    <div class="body-row">
      <div class="photo-col">
        ${image ? `<canvas id="sketchCanvas"></canvas>` : `<div class="no-photo">No Photo</div>`}
      </div>
      <div class="info-col">
  ...
  <script>
    ${
      ...
      // Real pencil-drawing look: edge detection (Sobel operator) turns
      // the photo into pure black line-art on white — no color, no
      // background tone, just the outlines a pencil would draw.
      function sobelSketch(ctx, w, h) {
        ...
        var gxK = [-1, 0, 1, -2, 0, 2, -1, 0, 1];
        var gyK = [-1, -2, -1, 0, 0, 0, 1, 2, 1];
        for (var y = 0; y < h; y++) {
          for (var x = 0; x < w; x++) {
            var val = 255;
            if (x > 0 && y > 0 && x < w - 1 && y < h - 1) {
              ...
              val = mag > 45 ? Math.max(0, 255 - mag * 1.8) : 255;
            }
            var idx = (y * w + x) * 4;
            out.data[idx] = val;
            out.data[idx + 1] = val;
      ...
      img.onload = function () {
        try {
          ...
          // Cap working size for speed — plenty sharp for a 3x3in label.
          var maxDim = 500;
