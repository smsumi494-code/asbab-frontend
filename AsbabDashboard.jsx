  ...
  const handleSend = async () => {
    ...
    try {
      const imageUrls = await Promise.all(
        images.map((img) => (img.file ? uploadImageToCloudinary(img.file) : img.url))
      );

      if (editingId) {
        const res = await authedFetch(`${API_BASE}/api/entries/${editingId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ rawText: text, imageUrls, moderator }),
        });
        ...
        const res = await authedFetch(`${API_BASE}/api/entries`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ rawText: text, imageUrls, moderator, group: groupId }),
        });
        if (!res.ok) throw new Error();
          ...
          <article key={entry.id} className="rounded-2xl overflow-hidden border border-[#241f17] bg-[#161310]">
            <div className="relative">
                ...
                <div className="flex overflow-x-auto snap-x snap-mandatory">
                  {entry.imageUrls.map((url, i) => (
                    <img
                      ...
                      src={url}
                      alt=""
                      className="w-full h-72 object-cover shrink-0 snap-center"
                      style={{ minWidth: "100%" }}
                    />
                  ))}
                </div>
              ...
              {entry.imageUrls && entry.imageUrls.length > 1 && (
                <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur px-2 py-0.5 rounded-md text-[10px] font-medium">
                  {entry.imageUrls.length}টা ছবি · সোয়াইপ করুন
                </div>
              )}
