              ...
              <div>
                ...
                <div className="space-y-2 mb-3">
                  ...
                  <p className="text-[10px] text-[#6b6152] mt-2">
                    ডেলিভারি নোট — Steadfast-এ পাঠানো হবে, রাইডার দেখতে পারবে (ঐচ্ছিক)
                  </p>
                  <textarea
...
// ---- HOME SCREEN --------------------------------------------------------
function HomeScreen({ entries, onOpenGroup, onOpenMenu, onOpenPurchaseCost, onOpenRecycleBin }) {
    ...
    <div className="min-h-screen bg-[#0f0d0a] text-[#f2ede4]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="divide-y divide-[#1c1913]">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-5 py-4 text-left active:bg-[#161310]"
        >
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-[#0f0d0a] shrink-0 bg-[#6b6152]">
            <Trash2 size={18} />
          </div>
          <div className="flex-1 min-w-0">
            <span className="font-medium text-[15px] text-[#f2ede4] truncate">Recycle Bin</span>
            <p className="text-[13px] text-[#6b6152] truncate mt-0.5">২৪ ঘণ্টা পর স্বয়ংক্রিয় মুছে যাবে</p>
          </div>
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#0f0d0a] text-[#f2ede4]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <header className="sticky top-0 z-30 bg-[#0f0d0a]/95 backdrop-blur border-b border-[#241f17] px-3 pt-4 pb-3 flex items-center gap-2">
        ...
        <h1 className="font-medium text-[15px] flex-1 truncate">Recycle Bin</h1>
      </header>
      ...
      <main className="max-w-lg mx-auto px-5 py-5 space-y-3">
        {loading ? (
          ...
          <p className="text-center py-16 text-[#5c5342] text-sm">Recycle Bin খালি</p>
        ) : (
          items.map((item) => (
    ...
    <>
      {view === "home" ? (
        <HomeScreen
          ...
          onOpenRecycleBin={() => setView("recycle_bin")}
        />
      ) : view === "purchase_cost" ? (
        <PurchaseCostScreen onBack={() => setView("home")} authedFetch={authedFetch} />
      ) : view === "recycle_bin" ? (
        <RecycleBinScreen onBack={() => setView("home")} authedFetch={authedFetch} />
      ) : (
        <GroupScreen
          groupId={view}
          entries={entries}
