...
import {
  ...
  Loader2, Paperclip, ChevronLeft, Menu, LogOut, UserPlus, Key, Shield, Lock,
} from "lucide-react";
            ...
            <>
              ...
              <div>
                ...
                <div className="space-y-2 mb-3">
                  ...
                  <p className="text-[10px] text-[#6b6152] mt-3">
                    SMS Token (bdbulksms.net থেকে — এই পেইজের নিজস্ব)
                  </p>
                  <input
                    value={pageSmsToken}
...
// ---- HOME SCREEN --------------------------------------------------------
function HomeScreen({ entries, onOpenGroup, onOpenMenu, onOpenPurchaseCost }) {
...
function PurchaseCostScreen({ onBack, authedFetch }) {
  ...
  // Which modal is open: null | "products" | "entry" | "payment" | "summary"
  ...
  const saveProduct = async () => {
    ...
    try {
      const body = JSON.stringify({ name: productName.trim(), pricePerUnit: Number(productPrice) });
  ...
  const saveEntry = async () => {
    ...
    try {
      const res = await authedFetch(`${API_BASE}/api/purchases/entries`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productId: Number(entryProductId), quantity: Number(entryQuantity) }),
      });
      if (res.ok) {
        setEntryProductId("");
        setEntryQuantity("");
        setModal(null);
        showToast("হিসাব যোগ হয়েছে");
      }
    } finally {
  ...
  const savePayment = async () => {
    ...
    try {
      const res = await authedFetch(`${API_BASE}/api/purchases/payments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ note: paymentNote.trim(), amount: Number(paymentAmount) }),
      });
      if (res.ok) {
        setPaymentNote("");
        setPaymentAmount("");
        setModal(null);
        showToast("পেমেন্ট যোগ হয়েছে");
      }
    } finally {
    ...
    <div className="min-h-screen bg-[#0f0d0a] text-[#f2ede4]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-lg mx-auto px-5 py-5 space-y-3">
        <button
          ...
          className="w-full bg-[#161310] border border-[#241f17] hover:border-[#b8935a] text-[#f2ede4] text-sm font-medium py-4 rounded-xl transition-colors"
        >
        ...
        <button
          ...
          className="w-full bg-[#b8935a] hover:bg-[#c9a56d] text-[#0f0d0a] text-sm font-semibold py-4 rounded-xl transition-colors"
        >
          টোটাল সামারি
        </button>
      </div>
      ...
      {modal === "entry" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
          <div className="w-full max-w-sm bg-[#1a1712] border border-[#3a3226] rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-serif text-lg text-[#f2ede4]">হিসাব যোগ করুন</h3>
      ...
      {modal === "payment" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
          <div className="w-full max-w-sm bg-[#1a1712] border border-[#3a3226] rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-serif text-lg text-[#f2ede4]">পেমেন্ট করুন</h3>
              <button onClick={() => setModal(null)} className="text-[#8a7a5c] hover:text-[#f2ede4]">
                <X size={20} />
              </button>
            </div>
      ...
      {/* ---- Total Summary modal (cash-memo style ledger) ---- */}
      {modal === "summary" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
          <div className="w-full max-w-sm bg-[#1a1712] border border-[#3a3226] rounded-2xl p-6 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-serif text-lg text-[#f2ede4]">টোটাল সামারি</h3>
              <button onClick={() => setModal(null)} className="text-[#8a7a5c] hover:text-[#f2ede4]">
                <X size={20} />
              </button>
            </div>
              ...
              <>
                <div className="space-y-2 mb-4">
                  {summary.ledger.map((item) => (
                    <div key={`${item.type}-${item.id}`} className="flex items-center justify-between bg-[#161310] border border-[#241f17] rounded-lg px-3 py-2">
                      <div className="min-w-0">
                        {item.type === "purchase" ? (
                      ...
                      <span className={`text-[13px] font-medium shrink-0 ml-2 ${item.type === "purchase" ? "text-[#e0a3a3]" : "text-emerald-300"}`}>
                        {item.type === "purchase" ? "+" : "−"}৳{item.amount}
                      </span>
                    </div>
                  ))}
                  {summary.ledger.length === 0 && (
                    <p className="text-center py-4 text-[#5c5342] text-xs">এখনো কোনো হিসাব নেই</p>
                  )}
                </div>

                <div className="border-t border-[#3a3226] pt-3 space-y-1">
...
// ---- GROUP SCREEN ---------------------------------------------------------
function GroupScreen({ groupId, entries, pages, fraudResults, refreshFraudResults, onBack, refreshEntries, moderator, authedFetch, isAdmin }) {
  ...
  // checks for a duplicate order elsewhere first, then asks "কোন পেইজের
  // পোস্ট?" if any pages exist.
  ...
  const filtered = websiteTabFiltered.filter((e) => {
                      ...
                      <div className="p-3">
                        ...
                        {s.sent ? (
                          <p className="text-[12px] text-emerald-300">
                            ✅ কুরিয়ারে পাঠানো হয়েছে — Parcel ID: {s.consignmentId} · ৳{s.amount || 0}
                          </p>
                        ) : s.status === "incomplete" ? (
  ...
  return (
    <>
      {view === "home" ? (
        <HomeScreen
          entries={entries}
          onOpenGroup={(id) => setView(id)}
          onOpenMenu={() => setDrawerOpen(true)}
          onOpenPurchaseCost={() => setView("purchase_cost")}
        />
      ) : view === "purchase_cost" ? (
        <PurchaseCostScreen onBack={() => setView("home")} authedFetch={authedFetch} />
      ) : (
        <GroupScreen
          groupId={view}
          entries={entries}
