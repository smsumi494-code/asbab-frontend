...
function SideDrawer({ open, onClose, auth, onLogout, authedFetch }) {
  ...
  const [newPageCourierKey, setNewPageCourierKey] = useState("");
  const [newPageCourierSecret, setNewPageCourierSecret] = useState("");
  const [newPageAiProvider, setNewPageAiProvider] = useState("google");
  const [newPageAiKey, setNewPageAiKey] = useState("");
  ...
  const addPage = async () => {
    if (!newPageName.trim() || !newPageCourierKey.trim() || !newPageCourierSecret.trim() || !newPageAiKey.trim()) return;
    setSavingPage(true);
    try {
      const res = await authedFetch(`${API_BASE}/api/pages`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          courierApiKey: newPageCourierKey.trim(),
          courierSecretKey: newPageCourierSecret.trim(),
          aiProvider: newPageAiProvider,
          aiApiKey: newPageAiKey.trim(),
        }),
      });
      if (res.ok) {
        setNewPageName("");
        setNewPageCourierKey("");
        setNewPageCourierSecret("");
        setNewPageAiKey("");
        loadPages();
      }
    } finally {
            ...
            <>
              ...
              <div>
                ...
                <p className="text-[11px] text-[#6b6152] mb-3">
                  প্রতিটা পেইজের নিজস্ব Courier ও AI key থাকবে। পোস্ট করার সময় কোন পেইজে যাবে জিজ্ঞেস করা হবে।
                </p>
                ...
                <div className="space-y-2 mb-3">
                  ...
                  <input
                    value={newPageCourierKey}
                    onChange={(e) => setNewPageCourierKey(e.target.value)}
                    placeholder="Courier Api Key"
                    className="w-full bg-[#17140f] border border-[#3a3226] rounded-lg px-3 py-2 text-sm placeholder-[#5c5342] focus:outline-none focus:ring-1 focus:ring-[#b8935a]"
                  />
                  <input
                    value={newPageCourierSecret}
                    onChange={(e) => setNewPageCourierSecret(e.target.value)}
                    placeholder="Courier Secret Key"
                    className="w-full bg-[#17140f] border border-[#3a3226] rounded-lg px-3 py-2 text-sm placeholder-[#5c5342] focus:outline-none focus:ring-1 focus:ring-[#b8935a]"
                  />
                  <p className="text-[10px] text-[#6b6152]">AI (অর্ডার পড়ার জন্য)</p>
                  <select
                    value={newPageAiProvider}
                    onChange={(e) => setNewPageAiProvider(e.target.value)}
                    className="w-full bg-[#17140f] border border-[#3a3226] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#b8935a]"
                  >
                  ...
                  <input
                    value={newPageAiKey}
                    onChange={(e) => setNewPageAiKey(e.target.value)}
                  ...
                  <button
                    ...
                    disabled={
                      savingPage ||
                      !newPageName.trim() ||
                      !newPageCourierKey.trim() ||
                      !newPageCourierSecret.trim() ||
                      !newPageAiKey.trim()
                    }
                    className="w-full flex items-center justify-center gap-2 bg-[#b8935a] hover:bg-[#c9a56d] disabled:opacity-60 text-[#0f0d0a] font-medium text-sm py-2.5 rounded-lg"
                  >
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {isAdmin && !entry.uploading && (
                  <div className="flex items-center gap-3">
                    {(groupId === "all_order" || groupId === "website_order") && (
