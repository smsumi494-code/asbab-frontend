...
<style>
  ...
  @media print {
    .photo-col img { filter: grayscale(100%) contrast(1.25) brightness(1.03) !important; }
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
          <div className="w-full max-w-sm bg-[#1a1712] border border-[#3a3226] rounded-2xl p-6">
            ...
            {fraudCheckFor.data && (
              <div>
                <p className="text-[11px] text-[#6b6152] mb-3">
                  📞 {fraudCheckFor.data.phone}
                  {fraudCheckFor.data.cached && <span className="ml-2 text-[#8a7a5c]">(সেভ করা ফলাফল, ৭ দিনের মধ্যে চেক করা)</span>}
                </p>
                ...
                {fraudCheckFor.data.steadfastOfficial && (
                  <div className="rounded-xl border border-[#3a3226] bg-[#17140f] px-4 py-3 mb-3">
                    ...
                    {fraudCheckFor.data.steadfastOfficial.message ? (
                      ...
                      <p className="text-[12px] text-[#8a7a5c]">
                        {fraudCheckFor.data.steadfastOfficial.error} ({fraudCheckFor.data.steadfastOfficial.current}/{fraudCheckFor.data.steadfastOfficial.limit})
                      </p>
                    ) : (
                      <p className="text-[12px] text-[#c9bfa8]">
                ...
                <p className="text-[11px] text-[#6b6152] mb-1">অন্যান্য দোকানের ডেটা (fraudbd.com)</p>

                {(() => {
                  const total = fraudCheckFor.data.totalSummary;
                  const rate = total?.successRate ?? 0;
                  const color =
                    total?.total === 0
                      ? "text-[#8a7a5c] border-[#3a3226] bg-[#17140f]"
                      : rate >= 80
                  ...
                  return (
                    <div className={`rounded-xl border px-4 py-3 mb-4 text-center ${color}`}>
                      <p className="text-2xl font-semibold">
                        {total?.total ? `${rate}%` : "নতুন কাস্টমার"}
                      </p>
                      <p className="text-[11px] mt-1">
                        {total?.total
                          ? `মোট ${total.total} · সফল ${total.success} · ক্যান্সেল ${total.cancel}`
                ...
                <div className="space-y-2">
                  {Object.entries(fraudCheckFor.data.Summaries || {}).map(([courier, info]) => (
                    <div
                      key={courier}
                      className="flex items-center justify-between bg-[#161310] border border-[#241f17] rounded-lg px-3 py-2 text-[12px]"
                    >
                      <span className="text-[#c9bfa8]">{courier}</span>
                      {info.data_type === "rating" ? (
                        <span className="text-[#8a7a5c]">{info.message}</span>
                      ) : (
                        <span className="text-[#8a7a5c]">
                          মোট {info.total} · সফল {info.success} · ক্যান্সেল {info.cancel}
                        </span>
                      )}
                    </div>
                  ))}
