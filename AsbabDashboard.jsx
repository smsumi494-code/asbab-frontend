...
// ---- HOME SCREEN --------------------------------------------------------
function HomeScreen({ entries, onOpenGroup, onOpenMenu, onOpenPurchaseCost, onOpenRecycleBin, onOpenAnalytics, recycleBinItems, isAdmin, allowedGroups, moderator }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div
            className="w-[66px] h-[66px] rounded-full flex items-center justify-center shrink-0 relative"
            style={{ border: "1.5px solid #C9A24B", background: "#0a0806", boxShadow: "0 0 18px rgba(201,162,75,0.25)" }}
          >
            <span className="absolute -top-[11px] text-[13px]" style={{ color: "#E8CD8A" }}>👑</span>
...
// ---- GROUP SCREEN ---------------------------------------------------------
function GroupScreen({ groupId, entries, pages, fraudResults, refreshFraudResults, onBack, refreshEntries, moderator, authedFetch, isAdmin, allowedGroups, appSettings, onNavigateTo }) {
  ...
  return (
    <div className="min-h-screen bg-[#0f0d0a] text-[#f2ede4] flex flex-col" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {(showComposeModal || editingId) && (
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/70 backdrop-blur-sm animate-overlay-in">
      ...
      {(groupId === "all_order" || groupId === "pending") && (
        <GroupBottomNav
          ...
          onAddOrder={() => {
            if (groupId === "all_order") setShowComposeModal(true);
            else onNavigateTo("all_order");
          }}
...
function GroupBottomNav({ active, onHome, onAddOrder, onProfile }) {
  const [chatToast, setChatToast] = useState(false);
  return (
    <div className="fixed bottom-3.5 left-1/2 -translate-x-1/2 w-[calc(100%-20px)] max-w-[420px] flex items-start justify-between z-40 rounded-[24px] px-2.5 pt-3.5 pb-3" style={{ background: "#0d0b08", border: "1px solid rgba(201,162,75,0.18)" }}>
      ...
      <button
        ...
        className="flex-1 flex flex-col items-center gap-1.5 pt-1 relative"
      >
        <div className="relative">
      ...
      {chatToast && (
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-[#1c1913] border border-[#3a3226] rounded-full px-4 py-2 text-[12px] text-[#f2ede4] whitespace-nowrap">
          শীঘ্রই আসছে
        </div>
      )}
    </div>
...
function BottomNav({ active, onNavigate }) {
  ...
  return (
    <div
      className="fixed left-1/2 -translate-x-1/2 bottom-3.5 w-full flex justify-around items-center py-[11px] px-1 z-30"
      style={{
        maxWidth: "384px",
...
// ---- PROFILE SCREEN -------------------------------------------------------
function ProfileScreen({ auth, onLogout }) {
  return (
    <div
    ...
    >
      ...
      <div className="px-5 pt-4 flex flex-col items-center">
        <div
          className="w-[88px] h-[88px] rounded-full flex items-center justify-center font-serif font-bold text-[30px] mb-3"
          style={{
...
function NotificationScreen() {
  return (
    <div
      className="min-h-screen pb-24 flex items-center justify-center text-center px-8 text-[#f2ede4]"
      style={{
        fontFamily: "'Inter', sans-serif",
        background: "radial-gradient(600px 300px at 100% 0%, rgba(201,162,75,0.10), transparent 60%), linear-gradient(180deg, #0c0a08 0%, #050403 100%)",
      }}
    >
      ...
      <div>
        ...
        <p className="font-serif text-[20px] mb-1">শীঘ্রই আসছে</p>
        <p className="text-[13px] text-[#8a8172]">অর্ডার/অ্যাক্টিভিটি নোটিফিকেশন ফিচারটি এখনো তৈরি করা হচ্ছে</p>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    <>
      {view === "home" ? (
        ...
        <ProfileScreen auth={auth} onLogout={handleLogout} />
      ) : view === "notification" ? (
        <NotificationScreen />
      ) : (
        <GroupScreen
      ...
      {["home", "notification", "profile"].includes(view) && (
        <BottomNav active={view} onNavigate={handleBottomNav} />
      )}
      <SideDrawer
        open={drawerOpen}
        onClose={() => {
