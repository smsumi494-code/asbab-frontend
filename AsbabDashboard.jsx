...
// A quick airy "whoosh" — built from filtered noise with a rising-then-
// falling sweep, which is what Messenger/Facebook's send sound actually
// is (air movement, not a musical beep). Their exact recording is
// copyrighted, so this is synthesized fresh, not a copy.
function playNotificationSound() {
  try {
    ...
    filter.frequency.exponentialRampToValueAtTime(2800, ctx.currentTime + duration * 0.45);
    ...
    // Some browsers block audio before any user interaction — safe to ignore
  }
}
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {isAdmin && !entry.uploading && (
                  <div className="flex items-center gap-3">
                    ...
                    {groupId !== "all_order" && groupId !== "making" && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
          <div className="w-full max-w-sm bg-[#1a1712] border border-[#3a3226] rounded-2xl p-6 text-center">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => {
                  const id = courierResultModal.entryId;
